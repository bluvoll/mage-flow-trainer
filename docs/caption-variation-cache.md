...
One main DDP rank performs encoding. A file lock serializes writers across training jobs, and SQLite WAL allows existing training readers to continue. Writes commit in batches of 64 embeddings, so an interrupted preprocessing run can resume with at most the last uncommitted batch to redo. Do not manually delete the SQLite WAL/SHM files while a process is using the database.
